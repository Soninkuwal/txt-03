...
bot = Client("bot",
             bot_token= "7771113355:AAGl_sLxKm_FXsKdlOw7SH96XhyuELYf0d4",
             api_id= 20945078,
             api_hash= "93f6b8ce4bb0ab61b4c7e42187f2aa64")
#auth_users id - 1234736828,1375282662
auth_users =  [7378826796]
...
async def account_login(bot: Client, m: Message):
    ...
    if input.document:
        ...
        await bot.send_document(-1002477655469, x)
    ...
    try:
        for i in range(count - 1, len(links)):
            ...
            try:
                cc = f'[ 🎬 ] **Vid ID : **{str(count).zfill(3)}\n**Video Title :** {name1} ({res})\n\n**Batch Name :** {b_name}\n\n**𝘿𝙤𝙬𝙣𝙡𝙤𝙖𝙙𝙚𝙙 𝘽𝙮 ➤** IPS LOVER 💖'
                cc1 = f'[ 📕 ] **Pdf ID : **{str(count).zfill(3)}\n**File Title :** {name1}\n\n**Batch Name :**{b_name}\n\n**𝘿𝙤𝙬𝙣𝙡𝙤𝙖𝙙𝙚𝙙 𝘽𝙮 ➤** IPS LOVER 💖'
                if "drive" in url:
                    try:
                        ka = await helper.download(url, name)
                        copy = await bot.send_document(chat_id=m.chat.id,document=ka, caption=cc1)
                        await copy.copy(chat_id = -1002352077049)
                        count+=1
                        os.remove(ka)
                        time.sleep(1)
